...
const customerLinks = [
	{ label: "Shop", to: "/shop", Icon: HomeIcon },
	...
	return (
		<div className="min-h-screen min-w-0 bg-canvas pb-20 md:pb-0">
			<header className="mx-auto flex min-w-0 max-w-6xl items-center justify-between gap-3 px-4 py-4 sm:px-7">
				...
				<div className="flex shrink-0 items-center gap-2"><p className="hidden rounded-full bg-surface px-3 py-2 text-xs font-medium text-muted sm:block">Delivery in 30–45 min</p><NavLink aria-label={cartLabel} className="inline-flex min-h-10 items-center gap-2 rounded-full bg-surface px-3 text-xs font-medium text-ink shadow-float" to="/cart"><ShoppingCartIcon aria-hidden="true" className="h-5 w-5" /><span>{itemCount}</span></NavLink></div>
			</header>
			...
			<nav aria-label="Customer navigation" className="fixed inset-x-3 bottom-3 z-10 grid min-w-0 grid-cols-5 rounded-card bg-ink p-1.5 text-surface shadow-float md:hidden">
				{customerLinks.map(({ label, to, Icon }) => (
					<NavLink className={({ isActive }) => `grid min-h-12 min-w-0 place-items-center gap-1 rounded-2xl py-1 text-[11px] font-medium ${isActive ? "bg-action text-ink" : "text-surface"}`} key={label} to={to}>
