...
export function OfferingForm({ mode }: { mode: "create" | "edit" }) {
	...
	if (mode === "edit" && offering.isError) return <section className="rounded-card bg-surface p-7 shadow-float"><h1 className="text-2xl font-black">Offering could not load.</h1><Button className="mt-5" onClick={() => void offering.refetch()} variant="secondary">Retry</Button></section>;
...
function OfferingEditor({ initialOffering, mode, offeringId, onReload, products }: { initialOffering?: Offering; mode: "create" | "edit"; offeringId?: string; onReload: () => void; products: Array<{ id: string; name: string }> }) {
	...
	return <div className="mx-auto max-w-5xl space-y-5 pb-28">
		<form className="space-y-5" noValidate onSubmit={(event) => void submit(event)}>
			<header className="rounded-card bg-ink p-5 text-surface shadow-float sm:flex sm:items-end sm:justify-between sm:p-7"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-action">Sellable offering</p><h1 className="mt-2 text-3xl font-black tracking-tight">{mode === "create" ? "New offering" : initialOffering?.sku}</h1><p className="mt-2 text-sm text-surface/70">Stock is adjusted separately so every count remains auditable.</p></div><Button className="mt-4 sm:mt-0" onClick={() => navigate("/cms/offerings")} type="button" variant="secondary">Back to offerings</Button></header>
			{notice ? <p aria-live="assertive" className="rounded-2xl border border-[#e8a28f] bg-[#fff0ea] px-4 py-3 text-sm font-bold text-[#8e301d]">{notice}{notice.includes("changed") ? <Button className="ml-3 min-h-8 px-3 text-xs" onClick={onReload} type="button" variant="secondary">Reload latest</Button> : null}</p> : null}
			<section className="rounded-card border border-white/70 bg-surface p-5 shadow-float sm:p-7"><div className="grid gap-4 sm:grid-cols-2">
				...
				<label className="grid gap-1.5 text-sm font-bold" htmlFor="offering-weight-unit">Weight unit<select className="min-h-12 rounded-2xl border border-line bg-surface px-4 font-normal" id="offering-weight-unit" onChange={(event) => set("weightUnit", event.target.value as Draft["weightUnit"])} value={draft.weightUnit}><option value="">No weight</option><option value="g">g</option><option value="kg">kg</option><option value="ml">ml</option><option value="l">l</option></select></label>
				...
				<label className="grid gap-1.5 text-sm font-bold" htmlFor="offering-discount-type">Discount type<select className="min-h-12 rounded-2xl border border-line bg-surface px-4 font-normal" id="offering-discount-type" onChange={(event) => set("discountType", event.target.value as Draft["discountType"])} value={draft.discountType}><option value="none">No discount</option><option value="fixed">Fixed</option><option value="percentage">Percentage</option></select></label>
				<Field disabled={draft.discountType === "none"} error={errors.discountValue} label={discountLabel} min="0" onChange={(event) => set("discountValue", event.target.value)} type="number" value={draft.discountValue} /><Field error={errors.lowStockThreshold} label="Low stock threshold" min="0" onChange={(event) => set("lowStockThreshold", event.target.value)} type="number" value={draft.lowStockThreshold} /><label className="flex min-h-12 items-center gap-3 rounded-2xl border border-line px-4 text-sm font-bold"><input checked={draft.active} className="size-5 accent-action" onChange={(event) => set("active", event.target.checked)} type="checkbox" />Active</label>
			</div></section>
