...
export function InventoryMovementsPage() {
	...
	if (movements.isError) return <section className="rounded-card bg-surface p-7 shadow-float"><h1 className="text-2xl font-black">Inventory ledger could not load.</h1><p className="mt-2 text-sm text-muted">{movements.error.message}</p><Button className="mt-5" onClick={() => void movements.refetch()} variant="secondary">Retry</Button></section>;
	return <div className="mx-auto max-w-7xl space-y-5"><header className="rounded-card bg-ink p-5 text-surface shadow-float sm:p-7"><p className="text-xs font-black uppercase tracking-[0.16em] text-action">Read-only inventory ledger</p><h1 className="mt-2 text-3xl font-black tracking-tight">Every stock change has a trace.</h1></header><section className="rounded-card border border-white/70 bg-surface p-5 shadow-float"><Field defaultValue={offeringId} label="Filter by offering ID" onChange={(event) => { const next = writeListState({ ...state, page: 1 }); if (event.target.value) next.set("offeringId", event.target.value); else next.delete("offeringId"); setParams(next); }} /></section><DataTable columns={columns} data={movements.data?.items ?? []} emptyMessage="No inventory movements match this view." isLoading={movements.isPending || movements.isFetching} onPageChange={(pageIndex) => setParams(writeListState({ ...state, page: pageIndex + 1 }))} onSortingChange={() => undefined} pageIndex={state.page - 1} pageSize={state.pageSize} rowCount={movements.data?.totalItems ?? 0} sorting={sorting} /></div>;
}
