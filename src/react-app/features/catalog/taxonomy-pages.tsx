...
function TaxonomyForm({ entity, kind, onCancel }: { entity: TaxonomyEntity | null; kind: TaxonomyKind; onCancel: () => void }) {
	...
	return <section aria-label={`${creating ? "Create" : "Edit"} ${entityName}`} className="rounded-card border border-white/70 bg-surface p-5 shadow-float sm:p-7">
		<div className="flex flex-wrap items-start justify-between gap-3 border-b border-line pb-4">
			<div><p className="text-xs font-medium uppercase tracking-[0.16em] text-action">Taxonomy record</p><h2 className="mt-1 text-xl font-medium tracking-tight">{creating ? `New ${entityName}` : `Edit ${entityName}`}</h2></div>
...
function TaxonomyPage({ kind }: { kind: TaxonomyKind }) {
	...
	const columns = useMemo<ColumnDef<TaxonomyEntity, unknown>[]>(() => [
		...
		{ accessorKey: "active", header: "Status", enableSorting: true, cell: ({ row }) => <span className={`rounded-full px-3 py-1 text-xs font-medium ${row.original.active ? "bg-[#e7f5df] text-[#28633e]" : "bg-[#f2f8fb] text-[#4f8194]"}`}>{row.original.active ? "Active" : "Inactive"}</span> },
	...
	if (result.isError) return <section className="rounded-card bg-surface p-7 shadow-float"><h1 className="text-2xl font-medium">{title} could not load.</h1><p className="mt-2 text-sm text-muted">{result.error.message}</p><Button className="mt-5" onClick={() => void result.refetch()} variant="secondary">Retry</Button></section>;

	return <div className="mx-auto max-w-7xl space-y-5">
		<header className="rounded-card bg-ink p-5 text-surface shadow-float sm:flex sm:items-end sm:justify-between sm:p-7"><div><p className="text-xs font-medium uppercase tracking-[0.16em] text-action">Catalog taxonomy</p><h1 className="mt-2 text-3xl font-medium tracking-tight">{title} keep the shelf intelligible.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-surface/70">Use consistent names and slugs so staff can organize the catalog and customers can find what they need.</p></div><Button className="mt-4 sm:mt-0" onClick={() => setEditing(null)} type="button">New {singular}</Button></header>
