...
export function CartPage() {
	...
	return <div className="grid min-w-0 gap-5 pb-4 lg:grid-cols-[1fr_22rem]">
		...
		<aside className="h-fit rounded-card bg-ink p-5 text-surface shadow-float"><h2 className="text-xl font-medium tracking-tight">Order summary</h2><div className="mt-5 flex items-center justify-between border-t border-surface/20 pt-4 text-lg font-medium"><span>Subtotal</span><span>{formatMoney(cart.data.subtotalMinor)}</span></div><p className="mt-4 rounded-2xl bg-surface/10 p-3 text-sm leading-6 text-surface/80">Checkout arrives in Phase 4. We will confirm delivery details and cash-on-delivery availability then.</p></aside>
	</div>;
}
