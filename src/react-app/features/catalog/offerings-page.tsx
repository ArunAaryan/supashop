...
export function OfferingsPage() {
	...
	const columns = useMemo<ColumnDef<Offering, unknown>[]>(() => [
		{ accessorKey: "sku", header: "SKU", enableSorting: true, cell: ({ row }) => <Link className="font-black underline-offset-4 hover:underline" to={`/cms/offerings/${row.original.id}`}>{row.original.sku}</Link> },
		...
		{ accessorKey: "effectivePriceMinor", header: "Effective price", enableSorting: true, cell: ({ row }) => <span className="font-black">{formatMoney(row.original.effectivePriceMinor)}</span> },
		{ accessorKey: "stockQuantity", header: "Stock", enableSorting: true, cell: ({ row }) => <span className={row.original.lowStock ? "font-black text-[#8e301d]" : "font-black"}>{row.original.stockQuantity}{row.original.lowStock ? " · Low" : ""}</span> },
		{ accessorKey: "active", header: "Status", cell: ({ row }) => <span className={`rounded-full px-3 py-1 text-xs font-black ${row.original.active ? "bg-[#e7f5df] text-[#28633e]" : "bg-[#fff0ea] text-[#8e301d]"}`}>{row.original.active ? "Active" : "Inactive"}</span> },
	], []);
	...
	if (offerings.isError) return <section className="rounded-card bg-surface p-7 shadow-float"><h1 className="text-2xl font-black">Offerings could not load.</h1><p className="mt-2 text-sm text-muted">{offerings.error.message}</p><Button className="mt-5" onClick={() => void offerings.refetch()} variant="secondary">Retry</Button></section>;
	return <div className="mx-auto max-w-7xl space-y-5">
		<header className="rounded-card bg-ink p-5 text-surface shadow-float sm:flex sm:items-end sm:justify-between sm:p-7"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-action">Sellable configurations</p><h1 className="mt-2 text-3xl font-black tracking-tight">Offerings make products ready for the shelf.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-surface/70">Set pack details and prices here. Stock changes remain in the inventory ledger.</p></div><Link className="mt-4 inline-flex min-h-11 items-center justify-center rounded-full bg-action px-5 text-sm font-bold text-ink shadow-[0_8px_0_#d95734] sm:mt-0" to="/cms/offerings/new">New offering</Link></header>
