...
function ProductImage({ product }: { product: ProductSummary }) {
	if (!product.primaryImage) return <span aria-label="No product image" className="grid size-11 place-items-center rounded-xl border border-dashed border-line bg-[#fff7f2] text-xs font-black text-muted">—</span>;
...
export function ProductsPage() {
	...
	const columns = useMemo<ColumnDef<ProductSummary, unknown>[]>(() => [
		...
		{ accessorKey: "code", header: "Code", enableSorting: true, cell: ({ row }) => <code className="text-xs font-black">{row.original.code}</code> },
		{ accessorKey: "name", header: "Product", enableSorting: true, cell: ({ row }) => <Link className="font-black underline-offset-4 hover:underline" to={`/cms/products/${row.original.id}`}>{row.original.name}</Link> },
		...
		{ accessorKey: "active", header: "Status", enableSorting: true, cell: ({ row }) => <span className={`rounded-full px-3 py-1 text-xs font-black ${row.original.active ? "bg-[#e7f5df] text-[#28633e]" : "bg-[#fff0ea] text-[#8e301d]"}`}>{row.original.active ? "Active" : "Draft"}</span> },
	...
	if (products.isError) return <section className="rounded-card bg-surface p-7 shadow-float"><h1 className="text-2xl font-black">Products could not load.</h1><p className="mt-2 text-sm text-muted">{products.error.message}</p><Button className="mt-5" onClick={() => void products.refetch()} variant="secondary">Retry</Button></section>;

	return <div className="mx-auto max-w-7xl space-y-5">
		<header className="rounded-card bg-ink p-5 text-surface shadow-float sm:flex sm:items-end sm:justify-between sm:p-7"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-action">Catalog master records</p><h1 className="mt-2 text-3xl font-black tracking-tight">Products start with a clear shelf story.</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-surface/70">Set the record, category, tags, and readiness before creating customer-facing offerings.</p></div><Link className="mt-4 inline-flex min-h-11 items-center justify-center rounded-full bg-action px-5 text-sm font-bold text-ink shadow-[0_8px_0_#d95734] transition hover:-translate-y-0.5 sm:mt-0" to="/cms/products/new">New product</Link></header>
