...
export function ProductGallery({ product }: { product: ProductDetail }) {
	...
	return <div className="space-y-5">
		{message ? <p aria-live="assertive" className="rounded-xl border border-[#e8a28f] bg-[#fff0ea] px-4 py-3 text-sm font-bold text-[#8e301d]">{message}</p> : null}
		<div className="grid gap-4 rounded-2xl border border-line bg-[#fff7f2] p-4 md:grid-cols-[minmax(0,1fr)_auto]">
			<div className="grid gap-3 sm:grid-cols-2">
				<label className="grid gap-1.5 text-sm font-bold" htmlFor="product-image-file">Image file<input accept="image/jpeg,image/png,image/webp,image/avif" disabled={busy || galleryFull} id="product-image-file" onChange={selectFile} type="file" /></label>
				<label className="grid gap-1.5 text-sm font-bold" htmlFor="product-image-alt">Alt text<input className="min-h-11 rounded-xl border border-line bg-surface px-3 font-normal outline-none focus:border-action" disabled={busy || galleryFull} id="product-image-alt" onChange={(event) => setAltText(event.target.value)} placeholder="Describe this image" value={altText} /></label>
			</div>
		...
		{galleryFull ? <p className="text-sm font-bold text-muted">Maximum of five images reached. Remove an image before uploading another.</p> : null}
